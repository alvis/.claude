...
- **`engineering-lease`** — the on-disk coordinator lease
  (`acquire | heartbeat | release | status | takeover`). Exactly one live
  token may write coordinator-owned state; a live foreign lease is never
  replaced; an expired lease yields only to an explicit, journaled takeover.
...
| `essential:takeover` | Resuming paused work, locally or from a receipt: checks leases, runs the doctor, resolves blocking decisions, and drives streams to their success criteria. |
