...
1. **Summary statistics** — initial sources, discovered sources (and across how
   many levels), total analyzed, duplicates detected, key findings count,
   research gaps count, overall confidence (High/Medium/Low), discovery depth
   reached (1-3), and credibility distribution
   (Academic / Official / Industry / Community counts).
2. **Executive summary** — 3-5 sentences covering the key discoveries.
3. **Key findings** — one subsection per finding:
...
7. **Research gaps** — what was not found or needs further investigation.
8. **References** — the complete source list with access information, grouped
...
Rate each finding by source credibility plus convergence: High needs multiple
independent credible sources agreeing; Medium is a single credible source or
mixed agreement; Low is community-only sourcing or unresolved contradiction.
The overall confidence in the summary statistics is the weighted impression
across key findings, not an average — one Low-confidence load-bearing finding
caps the overall rating at Medium.
