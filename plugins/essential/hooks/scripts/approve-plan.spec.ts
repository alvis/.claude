...
function runHook(input: Record<string, unknown>, harness = "codex", root?: string): ReturnType<typeof spawnSync> {
  ...
  environment.ESSENTIAL_APPROVED_PLAN_AUTOMATION = "0";
...
describe("approved plan hook delivery", () => {
  it.each(["codex", "claude"])("should inject approval context under %s despite the retired automation switch", (harness) => {
