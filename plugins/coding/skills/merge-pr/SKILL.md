...
- Never use plain `git rebase origin/main` for descendants. Always record the old parent tip before merging, then restack with `git rebase --onto <new-parent> <old-parent-tip> <child>` in git repos or the equivalent `jj rebase --revisions '<old-parent-tip>..<child-tip>' --destination <new-parent>` in jj repos.
...
3. **Verify one linear chain.** The stack is valid only when each downstream PR's `baseRefName` equals the previous PR's `headRefName`, and the first PR's base is the destination branch that will receive the stack. Also verify ancestry locally after fetching all heads. In plain git, use:
   ...
   For PR 1, verify `merge-base --is-ancestor origin/<base> origin/<head>`. In jj repos, verify the same relationships with jj revsets instead of switching to git history operations:

   ```bash
   jj log -r '<previous-head>@origin..<child-head>@origin' --no-graph
   jj log -r '<base>@origin..<head>@origin' --no-graph
   ```

   If any link fails, stop and show the expected chain as `base <- PR1 head <- PR2 head ...` plus the detected mismatch.
...
5. **Record old tips before the first merge.** Save every PR head SHA/change ID and branch or bookmark name in order. These old tips are the cut points for descendant rebases after GitHub rewrites or merges parent history:

   ```bash
   old_tip_<n>=$(git rev-parse origin/<head-branch>)        # plain git
   old_tip_<n>=$(jj log -r '<head-branch>@origin' --no-graph -T 'change_id')  # jj
   ```

6. **Merge and restack bottom-up.** For each PR from bottom to top:

   a. Recheck that the current PR is green unless `--force`, because prior restacks rerun CI.
   ...
   d. Restack every remaining downstream PR, one link at a time. For the immediate child of a PR merged into the destination branch, replay only child-exclusive commits onto `origin/<destination>` using the merged parent's recorded old tip. For deeper descendants, replay onto the freshly restacked parent branch/bookmark using that descendant's recorded old parent tip. Use the repository's native VCS path:

   ```bash
   git switch <child-head-branch>
   git rebase --onto <new-parent-ref> <old-parent-tip-sha>
   git push --force-with-lease origin HEAD:<child-head-branch>
   ```

   ```bash
   child_root=$(jj log -r 'roots(<old-parent-tip>..<child-tip>)' --no-graph -T 'change_id')
   ...
   e. After each push, update that child branch/bookmark's new local tip and use it as `<new-parent-ref>` for its child. Keep the original saved old tips unchanged for cut points.
...
8. **Conflict or failure recovery.** On rebase conflict, stop immediately and show the recovery commands for the active VCS:
   ...
   Include the conflicted branch/bookmark, old parent tip, intended new parent, and PR number. On push rejection, fetch and rerun the same native restack only after confirming the remote branch/bookmark was not updated by someone else.
...
- After every restack, verify ancestry with the active VCS:
  ...
  jj log -r '<new-parent-ref>..<child-head-branch>@origin' --no-graph
  ```
