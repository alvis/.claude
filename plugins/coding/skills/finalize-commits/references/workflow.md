...
Referenced from SKILL.md Step 2. Defines how the atomic per-commit walk runs as a dynamic Workflow when the `Workflow` tool is available, and the inline fallback when it is not. Per-commit procedure: `qa-loop.md`. All history mutations — folds, rewords, order surgery, the final head move, push — are applied by `coding:commit`; this file only orchestrates.
