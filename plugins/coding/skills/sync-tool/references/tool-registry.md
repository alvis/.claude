...
`sync-tool` is intentionally not a generic package manager. It exists to guarantee a known set of coding CLIs are present at minimum versions for sibling skills (e.g., `coding:commit` uses `jj` for history and `gh` for remote-state checks, while `coding:write-pr` uses both for publication). Keeping the registry small and explicit means each tool gets a hand-tuned, audited installer that matches its upstream's official guidance.
