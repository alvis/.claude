...
def run(slug: str, *, dry_run: bool = False) -> dict[str, object]:
    """Restack the open stack `slug` against its base ref.

    Pure-Python entry point used by both the CLI (`main`) and sibling scripts
    (e.g. `execute-stack.py`'s mandatory post-rewrite/post-follow-up restack).

    Returns a summary dict:
      {
        "slug": <slug>,
        "ops": [<line>, ...],                      # human-readable per-bookmark log
        "last_op_id": <jj op id>,
        "restacked_bookmarks": [<bookmark>, ...],  # bookmarks actually rebased+repushed
      }
    """
    root = repo_root()
    state = state_load(slug, root=root)
    if not state:
        print(f"no state file for slug={slug}", file=sys.stderr)
        return {"slug": slug, "ops": [], "last_op_id": "", "restacked_bookmarks": []}
    ...
    for pr in state.get("prs", []):
        bm = str(pr["bookmark"])
        if pr.get("status") == "merged" or is_merged(bm, dry_run=dry_run):
            pr["status"] = "merged"
    ...
    for pr in surviving:
        bm = str(pr["bookmark"])
        rebase(bm, new_base, dry_run=dry_run)
        push(bm, dry_run=dry_run)
        reparent_pr(bm, new_base, dry_run=dry_run)
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Pre-check: preserve the original CLI exit-code contract (2 == no state file).
    if not state_load(args.slug, root=repo_root()):
        print(f"no state file for slug={args.slug}", file=sys.stderr)
        return 2

    summary = run(args.slug, dry_run=args.dry_run)
    emit_json({
        "slug": summary["slug"],
        "ops": summary["ops"],
        "last_op_id": summary["last_op_id"],
    })
    return 0
