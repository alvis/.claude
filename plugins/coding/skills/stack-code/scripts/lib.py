...
def is_bookmark_merged(bookmark: str, *, dry_run: bool) -> bool:
    """Return True iff `gh pr view <bookmark> --json state` is exactly 'MERGED'.

    In dry-run, log the intended call and return False (treat as unmerged so the
    plan is exercised end-to-end). If `gh` exits non-zero (no PR found, auth
    failure, etc.) we conservatively return False so callers fall through to the
    normal unmerged-bookmark path; the merged-history guard in
    `references/workflow-correct.md` sub-flow C is enforced only on a confirmed
    `MERGED` state.
    """
    if dry_run:
        print(
            f"[DRY-RUN] would check merged state: gh pr view {bookmark} --json state -q .state",
            file=sys.stderr,
        )
        return False
    res = gh("pr", "view", bookmark, "--json", "state", "-q", ".state")
    if res.returncode != 0:
        return False
    return res.stdout.strip().upper() == "MERGED"


def squash_into_change(
    change_id: str,
    files: list[str] | None = None,
    *,
    dry_run: bool,
) -> None:
    """Move working-copy edits into the named ancestor change.

    With an explicit `files` list, runs `jj squash --into <change_id> <files...>`
    so only those paths are folded into the target change. With `files` omitted
    or empty, falls back to `jj absorb --into <change_id>` when supported, else
    plain `jj absorb` (the user accepted the absorb-distribution risk: plain
    `jj absorb` may scatter hunks to other ancestor changes that touched the
    same lines).
    """
    if files:
        jj_args = ["squash", "--into", change_id, "--", *files]
        res = jj(*jj_args, dry_run=dry_run)
        if res.returncode != 0 and not dry_run:
            raise SystemExit(f"jj squash --into {change_id} failed: {res.stderr.strip()}")
        return

    # Probe `jj absorb --help` once to see whether `--into` is supported on this jj build.
    if dry_run:
        print(
            f"[DRY-RUN] would absorb working-copy edits into {change_id} "
            "(jj absorb --into <change> when supported, else plain jj absorb)",
            file=sys.stderr,
        )
        return
    help_res = jj("absorb", "--help")
    supports_into = "--into" in help_res.stdout or "--into" in help_res.stderr
    absorb_args = ["absorb", "--into", change_id] if supports_into else ["absorb"]
    res = jj(*absorb_args, dry_run=dry_run)
    if res.returncode != 0:
        raise SystemExit(f"jj absorb failed: {res.stderr.strip()}")


def force_with_lease_push(bookmark: str, *, dry_run: bool) -> None:
    """Push a bookmark to its tracked remote (jj enforces lease semantics natively)."""
    # jj's `git push --bookmark` already refuses to clobber unrelated remote work; no extra flag needed.
    res = jj("git", "push", "--bookmark", bookmark, dry_run=dry_run)
    if res.returncode != 0 and not dry_run:
        raise SystemExit(f"jj git push --bookmark {bookmark} failed: {res.stderr.strip()}")
