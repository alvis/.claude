...
Referenced from `SKILL.md` step 8. Use this cycle only when this invocation owns the independent-review boundary selected there. Parent-owned deliveries return evidence for integrated review; publication alone defers independence to the PR stage.
...
4. On `blocked`, the implementing owner fixes the findings, reruns affected scans and project checks, then asks the assigned independent reviewer to verify the patch and affected findings under `coding:directions/review-evidence.md`. Allow about two correction rounds; unchanged evidence after that is a blocker, not permission to weaken a rule or check.
