# Subagent Mode (Agent Teams disabled, fallback)

Load this reference during Step 2 of the workflow when the session context does NOT contain `**Agent Teams**: enabled` under "Agent Capabilities". It documents the full Subagent-Mode workflow using fire-and-forget Task subagents with TodoWrite-based status tracking. If Agent Teams are enabled, use `references/team-mode.md` instead.

The original `### Step 2B:` header is preserved below so anchor references remain stable.
