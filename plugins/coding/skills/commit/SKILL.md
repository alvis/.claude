...
6. **Hand off publication after local work is complete.** With `--create-pr`, invoke `coding:push-pr <resolved-change-or-stack>` and forward `--branch-prefix <name>` and `--dry-run` when present. If `--no-verify` is also present, pass `--skip-local-test`. After any other local rewrite, identify affected unmerged bookmarks and invoke `coding:push-pr` with the resolved stack when they need remote restacking. Its [publication workflow](../push-pr/references/publish-stack.md) owns restack, push, and PR-base repair. Skip the handoff only when no remote publication was requested and no existing remote stack is affected.
