...
1. **Backup Repository**
   - Determine OS temp directory (`$TMPDIR` on macOS, `/tmp` on Linux)
   - Generate a numeric timestamp (epoch seconds, e.g. `date +%s`)
   - Copy the full working tree (all files, not just `.git`) to `<tmp>/git-backups/<repo-name>-<timestamp>` via `cp -R`
   - Never remove prior backups -- each commit session creates a new timestamped copy

2. **Baseline Tree Fingerprint**
   - Snapshot the intended final state as a git tree: `git -C <tmp>/git-backups/<repo-name>-<timestamp> add -A && git -C <tmp>/git-backups/<repo-name>-<timestamp> write-tree`
   - Record this tree SHA for post-commit integrity verification (Step 5)
...
3. **Rebase to squash fixups**:
   ...
   - If merge conflicts arise, consult the backup at `<tmp>/git-backups/<repo-name>-<timestamp>` as reference for the intended final state (the backup reflects the full final working tree -- resolution may only need a portion of the backup file). Attempt auto-resolution; if complex, delegate to a teammate to resolve, then continue the rebase
...
2. **Commit Splitting**
   ...
   - **Merge conflict resolution**: If merge conflicts arise during rebase, consult the backup at `<tmp>/git-backups/<repo-name>-<timestamp>` as reference for the intended final state. Note: the backup reflects the final working tree -- merge conflict resolution may only need a subset of the backup file's content, not necessarily the entire file.
...
> **Invariant**: The baseline tree SHA (Step 0) and `HEAD^{tree}` represent the same file set. They MUST be identical after a faithful commit chain. Any mismatch means content was lost or corrupted during commits/rebases.

1. **Compute post-commit tree**: `git rev-parse HEAD^{tree}`
2. **Assert equality** against the baseline tree SHA from Step 0
   ...
   **FAIL** — SHAs differ: this is a **hard failure**. Content was dropped, altered, or missed during the commit process.
   - Show divergent files: `GIT_ALTERNATE_OBJECT_DIRECTORIES=<tmp>/git-backups/<repo-name>-<timestamp>/.git/objects git diff-tree -r <baseline-tree> HEAD^{tree}`
   - Report each divergent file and the likely cause (rebase conflict mis-resolution, dropped hunk, staging error, unintended modification)
   - **STOP. Do NOT proceed to Step 6.** Present findings to the user and await instructions before continuing.
