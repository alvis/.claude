...
push:stack/01-a"
    ...
    edit-failure)
      ...
      expected_status=1
      expected_json='{"vcs":"jj","restacked":[],"skipped_merged":[],"errors":["pr-edit:stack/01-a"]}'
      expected_log="fetch
log:stack/01-a
discover:stack/01-a
log:stack/02-b
...
discover:stack/02-b"
  ...
  set +e
  output=$(cd "$COLOCATED" &&
    /bin/bash "$RESTACK_SH" ${args[@]+"${args[@]}"} 2>"$TMP_ROOT/stderr")
  status=$?
  set -e
...
printf 'PASS: 15 jj and 5 git fail-closed ordered stack sync cases\n'
