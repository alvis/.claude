...
6. **Merge and restack bottom-up.** For each PR from bottom to top:
   ...
   jj git push --remote "$REMOTE" \
     --bookmark <child-head-branch> \
     --bookmark <grandchild-head-branch>
   ```
   ...
   remote, then publish exactly once with repeated explicit selectors for all
   and only remaining affected bookmarks. Jujutsu's push safety checks are
   lease-like: rerun `jj git fetch --remote "$REMOTE"` and resolve bookmark
   conflicts if a remote bookmark changed unexpectedly.
