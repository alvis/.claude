...
**1. Verify delivery.** For a parent-owned delivery, apply [review ownership and evidence](review-evidence.md): children return self-checks, focused validation, and risks; the named parent owns one independent review after integration and documentation, before publication. Internal slice commits do not introduce extra independent passes. Without that explicit ownership contract, use the timing below. Confirm every requirement shipped: walk an executed plan task by task against code, tests, and docs; otherwise verify the stated requirements. Fix anything unmet, then restart the loop here. At Tier 0–1 the implementing owner verifies. When publication is the only review trigger for a bounded, non-consequential change, self-review before commit and let `coding:pr` supply the independent pass when no current independent evidence exists; do not add a separate local reviewer. Consequential changes, Tier 2–3, and explicit local review requests require independent pre-commit review through `coding:review-code`, which owns coverage and risk-based review staffing. Reuse applicable independent evidence across lifecycle and PR gates under `review-evidence.md`; publication still verifies the actual head/base, discussion, authorization, and CI. Coordination follows the implementation topology above; report categories never justify a coordinator.
