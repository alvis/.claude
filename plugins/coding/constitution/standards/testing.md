...
| beforeEach acceptable      | **Fresh instances preferred, no shared mutable state** |
...
Use `const` for immutable test data. Create fresh instances per test instead of reassigning variables.
...
Use constructor hijacking to return pre-defined mock instances:
...
- **No zero-parameter factories** - Use `const` instead
