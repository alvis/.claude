...
- **Substantial change** (worth a stacked PR) — `AskUserQuestion` where the work should live: the **current branch**, a fresh **local branch** in the current repo, a **`git worktree`**, or a **`jj` workspace**. Default path for a new worktree/workspace: `~/.workspaces/<project-root-folder-name>/<work-id>` (reuse the engineering work-id; the built-in `EnterWorktree` harness tool uses `.claude/worktrees/` and is not governed by this convention).
  - The work-id, the branch namespace, and that directory are one name: a work-id of `feat-work-id-naming` means directory `~/.workspaces/<project-root-folder-name>/feat-work-id-naming` and every branch under `feat/work-id-naming/` (first `-` becomes `/`). Each slice or sub-task of the stream is one numbered branch in that namespace — `feat/work-id-naming/01-code-spec`, `feat/work-id-naming/02-implementation`, `feat/work-id-naming/03-cleanup` — so the engineering resolver identifies the stream from any branch it is developed on. A branch shaped otherwise loses that, and every resolve falls back to asking the user.
  - **Never create the bare `feat/work-id-naming` branch**, even for a stream that starts as a single PR: git stores refs as files, so `feat/work-id-naming` and `feat/work-id-naming/01-code-spec` cannot both exist, and creating the bare one first blocks every later slice behind a rename. A one-PR stream is `01-<slice>` and grows from there.
