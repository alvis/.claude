...
- **Substantial change** (worth a stacked PR) — `AskUserQuestion` where the work should live: the **current branch**, a fresh **local branch** in the current repo, a **`git worktree`**, or a **`jj` workspace**. Default path for a new worktree/workspace: `~/.workspaces/<project-root-folder-name>/<work-id>` (reuse the engineering work-id; the built-in `EnterWorktree` harness tool uses `.claude/worktrees/` and is not governed by this convention).
  - The work-id, the branch, and that directory are one name: a work-id of `feat-work-id-naming` means directory `~/.workspaces/<project-root-folder-name>/feat-work-id-naming` and branch `feat/work-id-naming` (first `-` becomes `/`). Work that stays a single PR uses that branch and nothing more — no ordinal, no slice. A stream split into a stack or into sub-tasks becomes a set of two-digit-numbered branches beneath it: `feat/work-id-naming/01-resolver`, `feat/work-id-naming/02-contract`. Either way the engineering resolver identifies the stream from the branch that is checked out; a branch shaped otherwise resolves to nothing and the PM is asked instead.
  - Those two shapes cannot coexist — git stores refs as files, so `feat/work-id-naming` blocks `feat/work-id-naming/01-resolver` and vice versa. A single-PR stream that grows renames its branch into the namespace before adding the second slice. Full rules live in `naming.md` in the essential plugin's `references/` directory.
