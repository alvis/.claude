...
Media and creative production lifecycle: giving footage, renders, and campaign deliverables the same truth discipline code enjoys. Depends on `essential`. A production work stream is an ordinary work stream that declares a `media-project` or `asset-store` anchor (`essential/references/anchors.md`); this plugin adds the two things media work needs beyond that: **asset identity** and **revision-bound review**.
...
Both skills maintain versioned text manifests and review records — they never edit media. Reproducing any render is a technical operation: its manifest entry names the timeline revision, asset-manifest hash, render settings, and output hash it was built from.
