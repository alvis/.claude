...
For material mobile code, follow `essential:references/contractor-workflow.md`:
inspect the relevant screens, tests, configuration, dependencies, and design
handoff before asking, return the approval packet, and wait for approval. After
approval, implement the accepted native behavior; stop and report if a device,
platform, or design assumption fails.
