...
3. If improving existing: Call `list_pages`, then `new_page <url>` and `agent-browser --cdp <port> open <url>`, then capture current state via Chrome DevTools:
...
4. **Visual verification** — `new_page <preview-url>` and `agent-browser --cdp <port> open <preview-url>` for each preview; `take_screenshot` via chrome-devtools MCP to verify tokens render correctly and consistently.
