...
name: design
version: 4.3.0
description: 'Design modern, visually stunning UIs with layouts, palettes, typography, and animations; maintains a .design-<area-noun-phrase> task workspace with CONTEXT.md, DECISIONS.md, rendered HTML boards, and a root DESIGN.md general direction, plus an HTML preview catalog with WCAG contrast checks via Chrome DevTools MCP. Interactive direction picking with a local HTML candidate board, then area-by-area design selection: every page area gets a rendered board of N ranked variants (default N=3) sent as a realistic image, chosen one question at a time. Splits work across frontend-designer / frontend-implementer / frontend-evaluator specialists, then runs an independent evaluation loop that reconciles the built implementation against the design and submits side-by-side visual-diff artifacts (per area, light/dark, and full page) for final user confirmation; --facelift mode redesigns an existing site into a stunning v2 while preserving content and brand intent. Triggers when: "design the UI", "make this page beautiful", "redesign this component", "improve the visual design", "modernize the UI", "create a mockup", "facelift my site", "give this website a makeover", "make a v2 of this design". Also use when: proposing layouts, picking color palettes, adding visual polish, revamping an existing site''s look. Examples: "design a landing page", "make the dashboard look modern", "facelift https://example.com".'
argument-hint: "[page/component/site to design or improve] [--facelift] [--style=<style>] [--variants=<N>] [--skip-directions] [--quick]"
...
> **IMPORTANT — Confirm before you build.**
> You MUST present design options and get the user's explicit pick before implementing anything, unless the `--quick` flag is set.
> Never assume the user would accept all of the designs — a proposal is a candidate to be chosen, not a decision already made.
> Every direction and every page area passes through its board and its confirmation gate.
> Even with `--quick` (which auto-picks the top-ranked variant per area to skip the per-area questions), the choices are still surfaced for overturn at the Phase 3 sign-off gate — `--quick` speeds up selection, it never ships an unconfirmed design.
...
**Reusability is the key theme**: every visual decision lands in the CSS theme, every component decision favors the shared library, and every artifact must be resumable by an agent with zero context. Each task keeps its temporary boards, captures, previews, context, and detailed decision history under one `./.design-<area-noun-phrase>/` workspace; the root `DESIGN.md` carries the general direction and implementation handoff.
...
- Walk a page area by area (hero, nav, sections, footer), each chosen from a rendered image of N ranked variants (default N=3)
...
| `--variants=<N>` | Set the number of variants generated for each area/component and the connective-tissue board. Defaults to `N=3`; `N` must be an integer ≥2. The direction board remains 3–5 candidates. |
...
| `--quick` | Skip the per-area choice questions in `<area_boards>`. The direction board (unless `--skip-directions`) and Direction Summary confirmation still run; `N`-variant area boards are still generated and ranked, but the reviewer-ranked #1 variant is auto-picked per area, recorded in `.design-<area-noun-phrase>/DECISIONS.md`, and summarized in DESIGN.md §10 for overturn at the Phase 3 sign-off gate. |
...
`--variants=<N>` defaults to `N=3`; a non-integer or a value below 2 is invalid and must stop with a request for a valid value.
...
After the target is known, create or resume the task workspace described in
...
2. If an existing workspace is present, ask the user whether to resume the
   selected task or start a new `./.design-<area-noun-phrase>/` workspace. For
   a legacy `./.design` directory, explicitly ask whether to resume/migrate it
   or start a new named workspace. Never silently overwrite or merge workspaces.
3. For a new task, create `./.design-<area-noun-phrase>/CONTEXT.md` and
   `./.design-<area-noun-phrase>/DECISIONS.md` before asking design-direction
   questions or generating any board. For a resumed task, read both files before
   generating anything and continue from their recorded phase and next action.
4. Store every temporary design artifact — HTML, CSS, JSON, PNG, WebP, board,
...
the pick + rejections into `./.design-<area-noun-phrase>/DECISIONS.md` immediately, with a concise
...
1. **Generate N variants** of that area in the locked direction (`N=3` by default; set by
   `--variants=<N>`; team mode: style-explorers
...
3. **Build the board** at `./.design-<area-noun-phrase>/boards/<area-slug>.html` and
   keep its rendered image beside it at `./.design-<area-noun-phrase>/boards/<area-slug>.webp` per
...
5. **Ask ONE AskUserQuestion** (design-boards.md C3): "{Area}: which design?" — offer
   the top three ranked variants that exist (`#1 (Recommended)`, `#2`, and `#3` when
   present) plus "Another variant or mix — name the number(s)". If `N>3`, lower-ranked
   variants remain pickable through the mix/other option. Mix/other → build the merged
   variant, re-present, re-ask until an explicit pick.
6. **Record immediately** into `./.design-<area-noun-phrase>/DECISIONS.md`: the chosen
...
`--quick` short-circuits steps 4–5 only: `N`-variant boards are still generated
...
evaluation (Phase 5). Motion values (durations, easings, distances, staggers) come from
...
You are the Lead Orchestrator. Create a persistent team via `TeamCreate` of three
specialist roles (no seat names a model):

- **frontend-designer** — the design specialists that own Phases 1–3. Runs as
  3× style-explorer (each owns one aesthetic direction — palette + type + layout
  sketch) plus 1× visual-reviewer that audits candidates against design standards
  at plugins/web/constitution/standards/design/ and WCAG contrast rules and ranks
  every candidate/variant.
- **frontend-implementer** — the builder that owns Phase 4 (build) and every Phase 5
  rework pass. Works strictly inside the locked theming and component-reuse gates,
  from `DESIGN.md` + `./.design-<area-noun-phrase>/DECISIONS.md`.
- **frontend-evaluator** — the independent evaluator that owns the Phase 5
  reconciliation loop and the Phase 6 diff artifacts. Independence rules apply
  (see Phase 5): it receives the design spec and the rendered implementation, never
  the implementer's reasoning or self-assessment. It carries two lenses — design
  fidelity/quality critique and perf/a11y audit (the Design Critic + Perf/A11y
  Auditor roles from `references/facelift.md`).

Cycle (direction): lead briefs each frontend-designer style-explorer with a distinct
aesthetic constraint → the reviewer ranks all candidates against the standards → lead
runs the Direction Board per `<direction>` → user picks → chosen direction locks.
Cycle (area boards): for each area in `<area_boards>`, lead splits the `N` variants
across the style-explorers → reviewer ranks them (rank badges + why-this-rank) → lead
presents the board image and captures the pick → next area. Chosen designs iterate
with the reviewer until convergence.
Once the design is signed off (Phase 3), the frontend-implementer builds and the
frontend-evaluator reconciles it against the design (Phases 4–6) → `TeamDelete` on
completion.

In `--facelift` mode the frontend-evaluator runs the Design Critic + Perf/A11y Auditor
lenses per `references/facelift.md` — independence rules apply.
...
5. **Design layout** by assembling the chosen area variants, using component-type patterns and HCI principles to resolve the joins. Follow the recorded choices in `./.design-<area-noun-phrase>/DECISIONS.md`; do not re-litigate a confirmed decision.
...
The proposal is a design, not code. The frontend-implementer builds it in Phase 4 — only after the Phase 3 sign-off gate.
...
1. **Create or update `<root>/DESIGN.md`** — Use `references/design.template.md` for the overall design direction and implementation handoff. It must include the visual thesis, content plan, interaction thesis, agreed design tokens (colors, typography, spacing, radii, shadows, component styles), and the "Motion, Transitions & Separators" spec (§4) from the connective-tissue pick and World-Class Element Checklist. Keep the detailed candidate records in `./.design-<area-noun-phrase>/DECISIONS.md`; DESIGN.md §10 links to `CONTEXT.md` and `DECISIONS.md` and summarizes the chosen direction, per-area picks, rejected candidates, hard constraints, and AskUserQuestion outcomes. Keep the exact `/goal Follow the decisions in ./.design-<area-noun-phrase>/DECISIONS.md for this design task.` line near the top.
...
3. **Create `./.design-<area-noun-phrase>/previews/tokens/preview.html`** — From `references/preview.template.html`: a single self-contained catalog whose `@layer theme` block mirrors the project theme stylesheet values exactly, with the preview-only 3-state toggle.
4. **Visual verification** — run the Standard Capture Sequence (`<contrast_protocol>`) against `file://<root>/.design-<area-noun-phrase>/previews/tokens/preview.html`; `take_screenshot` in system, light, AND dark (set/remove `data-theme` via `evaluate_script`), saving each image as `screenshot.webp`, `screenshot-light.webp`, and `screenshot-dark.webp` beside the preview HTML under `./.design-<area-noun-phrase>/previews/tokens/`; run the contrast script in light and dark.
...
The **frontend-implementer** builds from the signed-off design — never before the Phase 3
sign-off gate.

1. **Build** the production code in the detected framework (React/Vue/Tailwind/vanilla),
   assembling the confirmed area picks and the World-Class Element Checklist per `DESIGN.md`
   and `./.design-<area-noun-phrase>/DECISIONS.md`. Consume only `--ui-*` and primitive
   tokens — never `--theme-*` (CSS-MODE-04) and no hardcoded hex/shadow literals.
2. **Capture the running implementation** — run the Standard Capture Sequence
   (`<contrast_protocol>`) against the built UI in light AND dark (set/remove `data-theme`
   via `evaluate_script`) at desktop and 375px, saving the screenshots under
   `./.design-<area-noun-phrase>/captures/`. These captures are the evaluator's inputs.

### Phase 5: Evaluate & Reconcile (INDEPENDENT LOOP UNTIL 10/10)

The **frontend-evaluator** — not the lead, not the implementer — audits the build against
the design. **Independence is the point**: the lead composes each `SendMessage` payload
from scratch containing ONLY the design spec (`DESIGN.md` + `DECISIONS.md` token and spec
tables), the design-reference renders (`boards/*.webp`, `previews/tokens/screenshot*.webp`),
and the implementation captures/URL from Phase 4. The evaluator never receives the
implementer's reasoning or self-assessment — one that reads the builder's rationale
inherits the builder's blind spots.

Each evaluation pass:

1. **Execute the Standard Capture Sequence** (MANDATORY — defined once in `<contrast_protocol>`; do not improvise the steps) whenever fresh captures are needed.
2. **Design-vs-implementation fidelity** — compare design intent to the actual build for every area × {light, dark}. Emit **specific, cited divergences**, each naming its source of truth and the gap ("hero: DECISIONS.md pick #2 specifies 3 scroll speeds; impl has 1"; "primary button focus-visible ring missing per DESIGN.md §4"). Vague verdicts ("looks great", "feels off") are invalid — bounce them back for specifics.
3. **MANDATORY: Element-level WCAG contrast check**:
...
4. **MANDATORY: Perceived readability check**:
...
5. **Lighthouse audit** (secondary signal):
   - Run `lighthouse_audit` for accessibility score
   - Note: Lighthouse results do NOT override findings from steps 3-4
   - If Lighthouse passes but screenshots show issues → the element fails
6. **Self-audit against `constitution/standards/design/scan.md`**:
...
7. **Verify the World-Class Element Checklist** (`<world_class_elements>`) row by row against the rendered result — page transition, scroll reveals, every section boundary's separator, hover AND focus-visible on every interactive element, reduced-motion, dynamic-region states; any missing row is a defect, not a nice-to-have
8. Score each of the 12 categories 1-10
9. **Verify token discipline** — implementation consumes only `--ui-*` and primitive tokens: grep component CSS for `--theme-` (must return zero matches — CSS-MODE-04) and for hardcoded hex/shadow literals; check rendered values match DESIGN.md.
10. Cross-check the implementation against the **Gotchas** table in `<verification>`. Any match requires immediate remediation.

**Reconcile.** The evaluator returns its cited divergences + failing categories to the **frontend-implementer** as the rework brief — the specific gap, not "make it better". The implementer loads the relevant rule file (`rules/des-*.md`) for targeted Fix guidance, fixes → re-renders → re-captures → hands back for re-evaluation.

**Exit predicate** — before leaving this phase, ALL of the following must be true:

- [ ] All 12 categories score 10/10
...
- [ ] No unresolved design-vs-implementation divergence remains
- [ ] Screenshot evidence of the passing state is captured
...
**Hard iteration budget: 5 evaluate↔implement rounds.** If the loop has not converged, stop and carry the residual divergences into the Phase 6 confirmation for the user to adjudicate — do not loop forever.

### Phase 6: Visual-Diff Confirmation

Show the user the design-vs-implementation delta and gate on their final approval.

1. **Build the diff artifacts** — under `./.design-<area-noun-phrase>/diffs/`, create one self-contained HTML board per comparison that places the design-reference render (from `boards/` or `previews/tokens/`) beside the matching implementation screenshot (from `captures/`), one artifact per mode. Inline CSS only, no build step, honor `prefers-reduced-motion`.
2. **Scale breadth to scope**:
   - Single component → one area × {light, dark}: `diffs/<area-slug>-light.html`, `diffs/<area-slug>-dark.html`.
   - Full page / facelift → a per-section board for each area plus a full-page comparison, each × {light, dark}: `diffs/<area-slug>-{light,dark}.html` and `diffs/full-page-{light,dark}.html`.

   Store each board's rendered image beside it at the matching `.webp`.
3. **Present** each board via the Part A4 mechanic (`references/design-boards.md`): `new_page file://<absolute path>` → `take_screenshot` → fix any breakage before the user sees it → `SendUserFile` with `display: render`, attaching the diff HTML as a secondary file. Caption each with its mode/area and any residual or intentionally-accepted divergence the evaluator flagged.
4. **Final confirmation gate** — one `AskUserQuestion` (safe default: approve as presented). Record the outcome in `./.design-<area-noun-phrase>/DECISIONS.md` and summarize in `DESIGN.md` §10. This is the skill's terminal user gate.

```
Phase 1 → Phase 2 → Phase 3 → Phase 4 → Phase 5 → Phase 6
Understand  Propose   Codify   Implement  Evaluate  Confirm
                                          ↑     |
                                          +-----+
                                       eval ↔ implement
```
...
| 12-category 10/10 frontend-evaluator score | required | required |
| Litmus checks + Gotchas | required | required |
| frontend-evaluator design-critic lens sign-off (cited exemplar divergence) | — | required |
| frontend-evaluator perf/a11y lens sign-off, metrics pasted in transcript | — | required |
...
| 11 | **Area Picks Locked** | Every enumerated area plus the connective-tissue board has a recorded pick in `./.design-<area-noun-phrase>/DECISIONS.md` and a summary in DESIGN.md §10 (`--quick`: auto-picks logged and surfaced at sign-off). |
...
- DESIGN.md is the overall design-direction and implementation handoff. Sections 10–13
  ...
  Steps, File Map) are MANDATORY. The active `./.design-<area-noun-phrase>/CONTEXT.md`
