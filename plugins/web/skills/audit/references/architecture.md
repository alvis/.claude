...
- For AI routing, the current typed-finding adapter maps `critical` to `p0`, `high` to `p1`, and every remaining input to `p2`.
- Canonical Markdown rendering reads the raw report severity and maps `critical|high|medium|low` to `P0|P1|P2|P3`; `info` remains advisory, as specified by `../templates/output.md`.
