...
_Template for implementing data controllers with validation, logging, and error handling_
...
class UserController implements DataController<User> {
  ...
  async upsert(data: UpsertData<User>): Promise<User> {
    ...
    try {
      ...
      this.logger.error("User upsert failed", {
        error,
  ...
  async bulkCreate(data: CreateData<User>[]): Promise<User[]> {
    ...
    try {
      ...
      const validatedData = data.map((item) =>
        this.validator.validateCreateData(item),
      );
      ...
      this.logger.error("Bulk user creation failed", {
        error,
        count: data.length,
      });
