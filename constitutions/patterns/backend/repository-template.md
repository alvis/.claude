...
*Template for implementing data access repositories following standard patterns*
...
class DatabaseUserRepository implements UserRepository {
  ...
  private buildWhereClause(filter: UserFilter): { where: string; params: unknown[] } {
  ...
  private buildSortClause(sort?: Array<{ field: string; order: 'asc' | 'desc' }>): string {
    if (!sort || sort.length === 0) {
      return 'ORDER BY created_at DESC';
    }

    const clauses = sort.map(({ field, order }) => `${field} ${order.toUpperCase()}`);
...
class TransactionalUserRepository implements UserRepository {
  ...
  private async setWithContext(user: User, ctx: TransactionContext): Promise<User> {
    if (user.id) {
