...
2. **Script Selection**:
   - Identify the relevant project meta file such as package.json
   - Identify any linting script in the meta file

3. **File Selection**:
   - Get list of staged files
   - Get list of unstaged files
   - Filter for code files (`.ts`, `.tsx`, `.js`, `.jsx`)
   - If more than 10 files, batch grouping them according to their file types (e.g. react vs ordinary typescript file) and project type (e.g. backend vs frontend), 10 files max per group

4. **Identify Applicable Workflows & Standards**
   - Identify any code review related workflows and standards by file names (not content)
...
1. **Primary Implementation**:
   - Launch the most suitable agents in parallel to review each batch group with the following input
     - The workflow and standards to follow
